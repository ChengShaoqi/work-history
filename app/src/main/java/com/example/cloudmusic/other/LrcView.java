...
public class LrcView extends View implements ILrcView {
     ...
     * 不高亮歌词的字体颜色
     */
    private int mNormalRowColor = Color.LTGRAY;

    /**
     * 拖动歌词时，在当前高亮歌词下面的一条直线的字体颜色
     **/
    private int mSeekLineColor = Color.DKGRAY;
    /**
     ...
     * 当没有歌词的时候展示的内容
     **/
    private String mLoadingLrcTip = "正在寻找歌词...";
...
//        }
    ...
    private void click(MotionEvent event) {
        ...
        float offsetX = x - mLastMotionX; //第一次按下的y坐标和目前移动手指位置的y坐标之差
        //如果移动距离小于10，不做任何处理
        if (Math.abs(offsetY) < mMinSeekFiredOffset && Math.abs(offsetX) < mMinSeekFiredOffset) {
            //系统自带的激活单击事件
            performClick();
            return;
        }
